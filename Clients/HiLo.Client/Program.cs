...
using (var channel = GrpcChannel.ForAddress("http://localhost:5277"))
{
    ...
    var readTask = Task.Run(async () =>
    {
        try
        {
            ...
            while (await responseStream.MoveNext(cts.Token))
            {
                ...
                Console.WriteLine(
                    $"[Update] {update.Type} - {update.Message} player={update.Player} attempts={update.Attempts} isOver={update.IsOver} winner={update.Winner}");
                if (update.IsOver)
                {
                    gameOver = true;
                    try
    ...
    while (!gameOver)
    {
        ...
        try
        {
            var reply = await client.GuessAsync(new GuessRequest
                { GameId = gameId, Player = playerName, Number = guessNumber });
            Console.WriteLine(
                $"Server: {reply.Result} (isOver={reply.IsOver} winner={reply.Winner} attempts={reply.Attempts})");
            if (reply.IsOver)
            {
                gameOver = true;
    ...
    await channel.WaitForStateChangedAsync(ConnectivityState.Shutdown);
}
